...
Block.prototype.rotated = function() {
    ...
    for (var x = 0; x < w; x++) {
        var row = [];
        for (var y = 0; y < h; y++) row[y] = this.matrix[y][w - x - 1];
        mat[x] = row;
    }
    return mat;
};
...
var blocks = [
    new Block().init([[1, 1, 1, 1]], '#c00'),
    new Block().init([[1, 1], [1, 1]], '#0c0'),
    new Block().init([[1, 1, 1], [1, 0, 0]], '#00c'),
    new Block().init([[1, 1, 1], [0, 0, 1]], '#00c'),
    new Block().init([[1, 1, 0], [0, 1, 1]], '#aa0'),
    new Block().init([[0, 1, 1], [1, 1, 0]], '#0aa'),
    new Block().init([[0, 1, 0], [1, 1, 1]], '#0aa')
];
// }}}
...
User.prototype.init = function(board, bricks) {
    ...
    window.onkeydown = function(e) {
        ...
        if (kc == 37 || kc == 65) {
            // left
            if (that.left_ok()) that.move_left();
...
User.prototype.left_ok = function() {
    ...
    for (var y = 0; y < this.block.matrix.length; y++) {
        if (this.block.matrix[y][0] == 0) continue;

        if (this.board[this.y + y][left] == 1) return false;
    }
...
User.prototype.right_ok = function() {
    ...
    for (var y = 0; y < this.block.matrix.length; y++)
        if (this.board[this.y + y][right] == 1) return false;

    return true;
};

User.prototype.move_left = function() {
    if (this.left_ok(this.board)) this.x -= 1;
};

User.prototype.move_right = function() {
    if (this.right_ok(this.board)) this.x += 1;
};

User.prototype.move_down = function() {
    if (this.y < this.board.length - 1) this.y += 1;
};
...
User.prototype.rotate_ok = function() {
    ...
    for (var y = 0; y < rotated.length; y++)
        for (var x = 0; x < rotated[0].length; x++)
            if (rotated[y][x] == 1 && this.board[this.y + y][this.x + x] == 1) return false;
    return true;
};
...
User.prototype.didHit = function(board) {
    ...
    for (var y = 0; y < matrix.length; y++)
        for (var x = 0; x < matrix[0].length; x++)
            if (
                matrix[y][x] == 1 &&
                (this.y + y + 1 >= board.length || board[this.y + y + 1][this.x + x] == 1)
            )
                return true;
...
User.prototype.draw = function(bricks) {
    ...
    for (var y = 0; y < matrix.length; y++)
        for (var x = 0; x < matrix[0].length; x++)
            if (matrix[y][x] == 1) bricks[this.y + y][this.x + x].className = 'current';
};
...
User.prototype.blockize = function(board, user) {
    ...
    for (var y = 0; y < matrix.length; y++)
        for (var x = 0; x < matrix[0].length; x++)
            if (matrix[y][x] == 1) board[this.y + y][this.x + x] = 1;
};
...
function draw_board(board, bricks) {
    for (var y = 0; y < board.length; y++)
        for (var x = 0; x < board[0].length; x++)
            ...
            //} else if (board[y][x] == 1) {
            if (board[y][x] == 1) bricks[y][x].className = 'block';
    //else if (board[y][x] == 2) {
    //bricks[y][x].className = 'current';
}

function clear_all(w, h, bricks) {
    for (var y = 0; y < h; y++) for (var x = 0; x < w; x++) bricks[y][x].className = 'space';
}
...
function clear_lines(lines, board) {
    if (lines.length == 4) console.log('!!!! TETRIS !!!!');

    for (var i = 0; i < lines.length; i++) {
        board.splice(lines[i], 1);

        var newline = [];
        for (var j = 0; j < board[0].length; j++) newline[j] = 0;
        board.unshift(newline);

        for (var j = i + 1; j < lines.length; j++) lines[j]++;
    }
}
...
function clear(bottom, board) {
    ...
    if (lines.length != 0) show_status(lines.length * 100);
}
...
function toggle() {
    if (playing) clearInterval(mainInterval);
    else {
...
function btn_click(kc) {
    if (kc == 37) {
        // left
        if (user.left_ok()) user.move_left();
        draw(user.board, user.bricks, user);
    } else if (kc == 39) {
        // right
        if (user.right_ok()) user.move_right();
        draw(user.board, user.bricks, user);
    } else if (kc == 38) {
        // up rotate
        if (user.rotate_ok()) {
